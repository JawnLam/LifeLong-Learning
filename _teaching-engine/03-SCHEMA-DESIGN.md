...
> **This document defines how each subject's atoms are designed. Different subjects have different structural shapes. This teaching engine does not impose a single atom taxonomy on all subjects; it provides the protocol by which each subject's cartridge defines its own.**

## What is an atom

An atom is the smallest testable, quizzable, linkable unit of knowledge in a subject. The word replaces "concept" (which is too narrow for subjects like cuisine or languages) and generalizes across all domain shapes.
...
## Universal (cross-subject) atom types
...
## Subject-specific atom types

Each cartridge defines its own atom types in `<Subject>/_schema.md`, following the protocol below.

### Protocol for designing subject-specific atoms

When cartridging a new subject, answer the following questions in `<Subject>/_schema.md` before creating any atoms:
...
**Q3: What is the smallest unit a quiz question would test?** That's the atomic unit. Anything smaller is a fragment; anything larger should decompose.

**Q4: What relationships exist between atoms?** Hierarchy, composition, similarity, contrast, cause-effect, historical sequence, prerequisite structure? These relationships become the `Related_Concepts:` fields in atom frontmatter and the `[[wiki-links]]` in note bodies.
...
**Q7: What subject-specific session activities, if any, does this subject need?** Beyond the six universal activities, does the subject need custom activity types (COOK-ALONG, SPEAK, PROVE, COMPOSE, PERFORM)? Each custom activity needs its trigger conditions specified.
...
2. **Answers to Q1–Q8** — the analytical answers that justify the atom choices
3. **Atom type definitions** — for each subject-specific atom type:
   - Name and plural
...
4. **Relationship vocabulary** — the named relations that link atoms (e.g., `prerequisite-of`, `variation-of`, `student-of`, `contains`)
5. **Mastery scale** — inherited 0–5 default or the subject's custom scale
6. **Custom session activities** — any activities beyond the universal six, with trigger conditions
7. **Folder structure** — how atoms are organized in subfolders within the cartridge
...
Inherited from `_teaching-engine/_templates/TEMPLATE-Session.md`. Universal across subjects. The `lll_Atoms_Engaged` field is subject-agnostic — it just lists atom slugs, regardless of their type.
...
- **File edits:** atom notes grow as mastery increases; session logs are append-only once written; `_state.md` is overwritten; quiz and SR-log files are append-only

## How the teaching engine uses `_schema.md`

When the assistant reads a cartridge, `_schema.md` tells it:

1. What atoms exist in this subject (so it knows what types of notes to create)
2. What each atom's required structure is (so it conforms to the schema)
3. What relationships can connect atoms (so it links correctly)
